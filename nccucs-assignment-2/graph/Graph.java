...
public class Graph<E> {
	...
	// connected by edges, and the edges have direction.
	// There cannot be more than one edge from a given node to another given
	// node, but there can be an edge from A to B and an edge from B to A.
	...
	public boolean addEdge(E parentNode, E childNode) {
		...
		if (!graphMap.containsKey(parentNode)
				|| !graphMap.containsKey(childNode)) {
	...
	public void checkRep() {
		Iterator<E> nodes = graphMap.keySet().iterator();
		while (nodes.hasNext()) {
			Object node = nodes.next();
			Assert.assertNotNull(node);
			Iterator<E> children = ((Set<E>) graphMap.get(node)).iterator();
			while (children.hasNext()) {
				Object child = children.next();
				Assert.assertTrue(graphMap.containsKey(child));
			}
		}
	}
